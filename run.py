...
class App:
    ...
    def classify_images(self):  
        ...
        for i, frame in enumerate(self.images):
            ...
            (inserted, not_inserted) = supply.predict(image)[0]

            label = 'inserted' if inserted > not_inserted else 'not_inserted'
            ...
            probability = inserted if inserted > not_inserted else not_inserted
            ...
            logger.success(f'Process ({i}): {label}')
        ...
        st.write(f'**Foram processadas: {total_images} imagens em {time_process} s**')
        ...
        st.write(f'*inserted:*')
        st.write(f'**{total_inserted} imagens -  {round((total_inserted*100) / total_images, 2)}%**')
        st.write(f'*not_inserted:*')
        st.write(f'**{total_not_inserted} imagens - {round((total_not_inserted*100) / total_images, 2)}%**')
...
if __name__ == '__main__':
    ...
    app = App(model='./model/keras_25.model', path='./images')
    app.run()